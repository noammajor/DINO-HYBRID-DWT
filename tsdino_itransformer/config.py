config = {
    ...
    # arithmetic — iTransformer does NOT patch. seq_len = num_patches * patch_len.
    "patch_len": 16,
    ...
    "window_step": 336,  # stride between windows (= window size → non-overlapping, full coverage)
    "num_patches": 21,   # → 336-timestep window (21*16=336), matches in-domain forecast seq_len
    "n_layers": 4,
    "n_heads": 16,
    "embed_dim": 128,
    "d_ff": 512,
    ...
    "mlm_phi":        0.0,         # DINO-ONLY for iTransformer (iBOT/MAE unsupported: no timestep-token axis)
    ...
    # ── Backbone: iTransformer (variate-token / inverted, cross-variate attn) ──
    # Each variate's full length-seq_len series is embedded into ONE token
    # (Linear(seq_len -> d_model)); the transformer self-attends ACROSS variates.
    # IMPORTANT: seq_len is FIXED by the inverted embedding, and the number of
    # variate tokens equals c_in — so pretrain and forecast must share seq_len,
    ...
    "itransformer_embed":           "timeF",
    "itransformer_freq":            "h",
    "itransformer_use_norm":        1,    # 1 = per-instance (Non-stationary) norm, per iTransformer
