...
class MultiWaveletCross(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, seq_len_q, seq_len_kv, modes, c=64,
        ...
        self.attn1 = FourierCrossAttentionW(in_channels=in_channels, out_channels=out_channels,
                                            seq_len_q=seq_len_q, seq_len_kv=seq_len_kv,
                                            modes=modes, activation=activation,
                                            mode_select_method=mode_select_method)
        self.attn2 = FourierCrossAttentionW(in_channels=in_channels, out_channels=out_channels,
                                            seq_len_q=seq_len_q, seq_len_kv=seq_len_kv,
                                            modes=modes, activation=activation,
                                            mode_select_method=mode_select_method)
        self.attn3 = FourierCrossAttentionW(in_channels=in_channels, out_channels=out_channels,
                                            seq_len_q=seq_len_q, seq_len_kv=seq_len_kv,
                                            modes=modes, activation=activation,
                                            mode_select_method=mode_select_method)
        self.attn4 = FourierCrossAttentionW(in_channels=in_channels, out_channels=out_channels,
                                            seq_len_q=seq_len_q, seq_len_kv=seq_len_kv,
                                            modes=modes, activation=activation,
                                            mode_select_method=mode_select_method)
...
class FourierCrossAttentionW(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        xqkvw  = self.compl_mul1d("bhxy,bhey->bhex", xqk_ft, xk_ft_)
...
class sparseKernelFT1d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.fft.irfft(out_ft, n=N).permute(0, 2, 1).view(B, N, c, k)
        return x
