...
# vendored TS2Vec encoder lives in this variant's models/ — no ts2vec-main on path needed
from models.ts2vec_backbone import TS2VecForDINO as PatchTST, PatchReconDecoder
...
def train_TS_DINO(args):
    ...
    if use_reconstruction:
        student_recon = PatchReconDecoder(embed_dim, args.c_in).to(device)
    ...
    if use_mlm and mlm_mode == "ibot":
        ...
        # TS2Vec is channel-mixed: each masked timestep token predicts its C raw values.
        _mae_out = args.c_in
    ...
    for it, batch in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            if use_mlm:
                ...
                _bbone = student_without_ddp.backbone           # TS2VecForDINO model
                _NP    = _T                                     # TS2Vec: timestep tokens
                _pmask = torch.rand(_B, _NP, device=device) < mlm_mask_ratio
                # Student always replaces masked timesteps with mask_token
                _s_tok = _bbone.forward_ibot(_mlm_in, _pmask)   # [B, T, 1, d_model]
                ...
                if _s_masked.shape[0] > 0:
                    if mlm_mode == "ibot":
                        ...
                        # TS2Vec channel-mixed: each masked timestep token predicts its C raw values.
                        _gt_all    = _mlm_in.reshape(_B2*_NV, _NP2, _C)      # n_vars=1 → [B, T, C]
                        _gt_masked = _gt_all[_mexp]                          # [N_masked, C]
                        _pred      = student_mae_head(_s_masked)             # [N_masked, C]
