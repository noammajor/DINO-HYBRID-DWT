"""MAE pretraining of the TimeMixer backbone (standalone flow).

Block-masked reconstruction: split each window into blocks of --block_len
timesteps, mask --mask_ratio of them, reconstruct the masked blocks (PatchTST
PretrainHead design). Saves the backbone under the ``encoder.*`` keys so it can
be transferred to a downstream model.

Example:
  python train.py --data_path ../data/ETTh1.csv --c_in 7 \
      --seq_len 336 --block_len 8 --mask_ratio 0.4 --output_dir ./checkpoints/etth1
"""
import os
import sys
import argparse
...
import torch
from model import TimeMixerMAE
...
def get_args():
    ...
    p.add_argument('--output_dir', default='./checkpoints/mae')
    p.add_argument('--c_in', type=int, default=7)
    p.add_argument('--seq_len', type=int, default=336)
    p.add_argument('--block_len', type=int, default=8, help='timesteps per MAE block')
...
def main():
    ...
    model = TimeMixerMAE(
        c_in=args.c_in, seq_len=args.seq_len, block_len=args.block_len,
        mask_ratio=args.mask_ratio, d_model=args.d_model, e_layers=args.e_layers,
        ...
        top_k=args.top_k, use_norm=args.use_norm,
    ...
    print(f"[mae] params={sum(p.numel() for p in model.parameters()):,}  "
          f"blocks/window={model.n_blocks}")
    ...
    print(f"[mae] done. best={best:.6f}  ckpt dir={args.output_dir}")
