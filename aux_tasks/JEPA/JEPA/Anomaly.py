...
_SHARED = str(_Path(__file__).resolve().parents[3] / "shared")
...
def anomaly_detection(self, path, anomaly_train, anomaly_test,
    ...
    print(f"  [JEPA] threshold={threshold:.6f}  (top {anomaly_ratio}%)")
    print(am.format_table(m, title="JEPA Anomaly Detection"))
