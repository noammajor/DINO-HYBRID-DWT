config = {
    ...
    "n_layers": 10,     # TS2Vec encoder depth (# dilated residual blocks); driven by --encoder_layers in sweeps
    "ts2vec_hidden_dims": 64,  # TS2Vec internal conv hidden width (output_dims = embed_dim)
    "n_heads": 16,      # unused by TS2Vec (kept for config-schema parity)
    "embed_dim": 128,   # TS2Vec output_dims / representation dim fed to the DINO head
    "d_ff": 512,        # unused by TS2Vec (kept for config-schema parity)
    "dropout": 0.1,
    "head_dropout": 0.1,
