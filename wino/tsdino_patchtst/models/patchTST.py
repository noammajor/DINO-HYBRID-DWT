...
from tsdino_common.util import get_activation_fn
from models.layers.attention import *
from tsdino_common.util import DropPath
...
class PatchTST(nn.Module):
    ...
    def forward_ibot(self, z, mask=None):
        """iBOT/MAE patch encoding — each patch is a token.

        z:    [bs, seq_len, n_vars]
        mask: [bs, num_patch] bool — True=masked (student uses mask_token). None=full pass (teacher).
        returns: [bs, num_patch, n_vars, d_model]
        Uses non-overlapping patches (step = patch_len). No RevIN applied.
        """
        patches_tensor = z.unfold(dimension=1, size=self.patch_len, step=self.patch_len)
        tokens = self.backbone(patches_tensor, mask_patches=mask)  # [bs, num_patch+1, n_vars, d_model]
        return tokens[:, 1:, :, :]              # drop CLS  [bs, num_patch, n_vars, d_model]
...
class PatchTSTEncoder(nn.Module):
    def __init__(self, c_in, num_patch, patch_len, 
        ...
        # iBOT/MAE mask token — learnable embedding placed at masked patch positions
    ...
    def forward(self, x, padding_mask=None, mask_patches=None) -> Tensor:
        """
        x: tensor [bs x num_patch x nvars x patch_len]
        padding_mask: [bs x num_patch] bool, True=real data (optional, only for classification)
        mask_patches: [bs x num_patch] bool, True=masked → replaced with mask_token (iBOT/MAE student pass)
        """
        ...
        # iBOT/MAE masking: replace masked patch embeddings with the learnable mask_token
        if mask_patches is not None:
            _m = mask_patches.to(x.device).bool().unsqueeze(-1).unsqueeze(-1)    # [bs x num_patch x 1 x 1]
            x = torch.where(_m, self.mask_token.view(1, 1, 1, self.d_model), x)
