...
from models.timerxl import TimerXL, PatchReconDecoder
...
def train_TS_DINO(args):
    ...
    # ── MLM auxiliary branch (iBOT / MAE) — same as the TimeMixer backbone ──────
    ...
    if use_mlm and mlm_mode == "ibot":
        ...
        # PatchTST: predict the patch_len raw values of each masked patch token.
        _mae_out = args.patch_len
...
#----------------Train Loop --------------------
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if use_mlm and student_ibot_head is not None:
            save_dict['student_ibot_head'] = student_ibot_head.state_dict()
            save_dict['teacher_ibot_head'] = teacher_ibot_head.state_dict()
        if use_mlm and student_mae_head is not None:
            save_dict['student_mae_head'] = student_mae_head.state_dict()
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader, optimizer, epoch, fp16_scaler, lr_schedule, wd_schedule, momentum_schedule, args, student_recon=None, teacher_recon_decoder=None, use_mlm=False, mlm_mode="ibot", mlm_phi=0.0, mlm_mask_ratio=0.4, student_ibot_head=None, teacher_ibot_head=None, ibot_center=None, student_mae_head=None):
    ...
    if student_mae_head is not None:
    ...
    for it, batch in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            if use_mlm:
                ...
                _bbone = student_without_ddp.backbone           # PatchTST model
                _NP    = _bbone.num_patch                       # patch-level tokens
                _pmask = torch.rand(_B, _NP, device=device) < mlm_mask_ratio
                # Student always replaces masked patches with mask_token
                _s_tok = _bbone.forward_ibot(_mlm_in, _pmask)   # [B, NP, n_vars, d_model]
                _B2, _NP2, _NV, _DM = _s_tok.shape
                # Row order must match the GT/teacher reshape below: (B, n_vars, NP).
                ...
                if _s_masked.shape[0] > 0:
                    if mlm_mode == "ibot":
                        ...
                        # PatchTST: each masked patch token predicts its patch_len raw values.
                        _patches   = _mlm_in.unfold(dimension=1, size=_bbone.patch_len,
                                                    step=_bbone.patch_len)   # [B, NP, n_vars, patch_len]
                        _gt_all    = _patches.permute(0, 2, 1, 3).reshape(_B2*_NV, _NP2, _bbone.patch_len)
        ...
        with torch.no_grad():
            ...
            if use_recon_this:
                for param_q, param_k in zip(student_recon.parameters(), teacher_recon_decoder.parameters()):
                    param_k.data.mul_(m).add_((1 - m) * param_q.detach().data)
            # EMA update for the iBOT teacher head
            if use_mlm and student_ibot_head is not None:
...
def test_run(args):
    ...
    _SEQ_LEN = 336   # 21 patches × 16 = matches the 336 pretraining context
    ...
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    _forecast_num_patch = _SEQ_LEN // args.patch_len   # e.g. 512 // 16 = 32
    model = TimerXL(
        c_in= args.c_in,
        target_dim=args.pred_len,
        patch_len=args.patch_len,
