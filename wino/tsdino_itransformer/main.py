...
from models.itransformer_backbone import iTransformerForDINO, iTransformerForecastModel, PatchReconDecoder
...
def train_TS_DINO(args):
    ...
    #------------- Student - Teacher network (iTransformer backbone) -----------
    # iTransformer is a VARIATE-TOKEN / inverted transformer with cross-variate
    # attention: it embeds each variate's full length-seq_len series into ONE token
    # (Linear(seq_len -> d_model)), then self-attends ACROSS variates.
    # NOTE: seq_len is FIXED by the inverted embedding, and the number of variate
    # tokens equals c_in — so both seq_len and channel count are baked in here.
    # (In-domain pretraining, so this is fine.)
    _seq_len = args.num_patches * args.patch_len
    _it_kwargs = dict(
        c_in=args.c_in,
        d_model=cfg.get('itransformer_d_model', 128),
        e_layers=cfg.get('itransformer_e_layers', 2),
        d_ff=cfg.get('itransformer_d_ff', 256),
        n_heads=cfg.get('itransformer_n_heads', 8),
        dropout=args.dropout,
        factor=cfg.get('itransformer_factor', 1),
        activation=cfg.get('itransformer_activation', 'gelu'),
        embed=cfg.get('itransformer_embed', 'timeF'),
        freq=cfg.get('itransformer_freq', 'h'),
        seq_len=_seq_len,
        # Instance-normalize per iTransformer (Non-stationary Transformer style).
        use_norm=cfg.get('itransformer_use_norm', 1),
    )
    student = iTransformerForDINO(**_it_kwargs)
    teacher = iTransformerForDINO(**_it_kwargs)
...
def test_run(args):
    ...
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    # iTransformer forecast backbone. seq_len is FIXED by the inverted embedding and
    # must match the pretraining seq_len; c_in fixes the number of variate tokens.
    # These MUST match the pretrain _it_kwargs so the checkpoint loads.
    _it_kwargs = dict(
        c_in=args.c_in,
        d_model=cfg.get('itransformer_d_model', 128),
        e_layers=cfg.get('itransformer_e_layers', 2),
    ...
    model = iTransformerForecastModel(
        backbone=iTransformerForDINO(**_it_kwargs),
